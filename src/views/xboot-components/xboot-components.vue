<template>
  <div>
    <Card>
      <Layout>
        <Sider hide-trigger style="flex: 0 0 220px; max-width: 220px; background: #fff">
          <Menu
            active-name="1-0"
            theme="light"
            width="auto"
        ...
        >
          <div v-show="currName == '1-0'">
            <Alert type="warning" show-icon>
              说明：大部分组件为包含真实数据接口的简单封装，方便大家的直接复用！
            </Alert>
            <Divider class="blue" orientation="left">全局Loading加载动画</Divider>
            <Button @click="showLoading">显示右上角加载动画</Button>
            <Button style="margin-left: 5px" @click="closeLoading">关闭右上角加载动画</Button>
            <h3 class="article">使用方式</h3>
            修改全局Vuex即可，开启：
            <code>this.$store.commit("setLoading", true)</code>
            关闭：
            ...
            <Button style="margin-left: 5px" @click="finish">结束加载</Button>
            <Button style="margin-left: 5px" @click="error">错误</Button>
          ...
          <div v-show="currName == '1-1'">
            ...
            <count-down-button count-time="10" />
            <h3 class="article">提示</h3>
            你可以将
            <code>autoCountDown</code>
            属性设置为
            <code>false</code>
            ，即可手动调用开始倒计时方法
          ...
          <div v-show="currName == '1-12'">
            ...
            <div style="margin-bottom: 10px; font-size: 12px">
              获取用户列表示例（请求地址url="/user/getAll" 指定绑定的value字段valueBind="id"
              指定标题字段title="nickname" 指定备注字段description="username"）：
            </div>
            <code>{{ customList1 }}</code>
            <br />
            <br />
            <div style="margin-bottom: 10px; font-size: 12px">
              获取角色列表示例（请求地址url="/role/getAllList" 指定绑定的value字段valueBind="name"
              指定标题字段title="name" 指定备注字段description="description"）：
            </div>
            <customList
              v-model="customList2"
              url="/role/getAllList"
            ...
            <code>请求url</code>
            、
            <code>绑定值valueBind</code>
            、
            <code>标题title</code>
            、
            <code>备注description(可选)</code>
            属性获取指定接口数据， 使用
            <code>v-model</code>
            实现数据的双向绑定。
            <h3 class="article">props</h3>
            <Table :columns="props" :data="data40" border size="small" width="1000"></Table>
            <h3 class="article">events</h3>
            <Table :columns="events" :data="data41" border size="small" width="1000"></Table>
          </div>
          ...
          <div v-show="currName == '1-5'">
            ...
            <user-choose ref="user" text="点我选择用户" @on-change="handleSelectUser"></user-choose>
            <br />
            {{ selectUsers }}
            <h3 class="article">props</h3>
          ...
          <div v-show="currName == '1-13'">
            ...
            <br />
            <br />
            <Button style="margin-right: 8px" @click="setUserData">setData()赋值</Button>
            <h3 class="article">props</h3>
            <Table :columns="props" :data="data42" border size="small" width="1000"></Table>
            <h3 class="article">events</h3>
            <Table :columns="events" :data="data43" border size="small" width="1000"></Table>
            <h3 class="article">methods</h3>
            <Table :columns="methods" :data="data44" border size="small" width="1000"></Table>
          </div>
          ...
          <div v-show="currName == '1-10'">
            ...
            基本用法，使用
            <code>v-model</code>
            实现数据的双向绑定。绑定值对象：{url: 完整下载链接, name: 文件名, size:
            文件大小(单位字节，非必须)}
            <h3 class="article">props</h3>
          ...
          <div v-show="currName == '3-1'">
            <Alert type="info" show-icon>
              ...
              封装，已配置好图片上传(上传至XBoot文件服务或Base64)、表情包。扩展编辑HTML代码、全屏预览、清空、XSS攻击过滤等。
            </Alert>
            ...
            <code>FormItem</code>
            中使用时，建议加上
            <code>class="form-wangEditor"</code>
            。参考代码生成器中的用法。
            <h3 class="article">props</h3>
          ...
          <div v-show="currName == '2-1'">
            <Alert type="warning" show-icon>
              流程审批通用状态及结果保存至ActBusiness表中，添加业务时记得向该表中关联表ID等数据，参考开发文档工作流部分
            </Alert>
            ...
            <h3 class="article">props</h3>
            <Table :columns="props" :data="data15" border size="small" width="1000"></Table>
            <h3 class="article">events</h3>
            <Table :columns="events" :data="data14" border size="small" width="1000"></Table>
          </div>
          <div v-show="currName == '2-2'">
            <Divider class="blue" orientation="left">通过流程key直接发起最新版本</Divider>
            <div style="margin-bottom: 10px; font-size: 12px">
              示例（此处传入的属性流程key为请假申请leave，传入actBusinessId为测试数据123456，所以会报错）：
            </div>
            <Button :loading="processLoading" @click="showProcessStart = true">发起流程</Button>
            <h3 class="article">基础用法</h3>
            基本用法，使用
            <code>v-model</code>
            实现数据的双向绑定（组件显示），显示组件前记得传入有效属性值：processKey和actBusinessId。
            <h3 class="article">props</h3>
            <Table :columns="props" :data="data17" border size="small" width="1000"></Table>
            <h3 class="article">events</h3>
          ...
          <div v-show="currName == '2-3'">
            ...
            <h3 class="article">props</h3>
            <Table :columns="props" :data="data19" border size="small" width="1000"></Table>
            <h3 class="article">events</h3>
    ...
    <!-- 流程选择组件 -->
    <process-choose v-model="showProcessChoose" act-business-id="123456" />

    <process-start
      v-model="showProcessStart"
      process-key="leave"
      act-business-id="123456"
      @on-loading="processLoading = true"
      @on-loaded="processLoading = false"
    />

    <process-cancel v-model="showProcessCancel" act-business-id="123456" proc-inst-id="123456" />
  </div>
</template>
  ...
  data() {
    return {
      ...
      data9: [
        {
          name: "on-change",
          type: "返回选择用户数组",
          value: '选择用户数组Array，包含用户id和username，例如 [{"id":"1","username":"name"}]',
        },
      ],
      data10: [
        {
          name: "setData",
          type: "设置已选用户数据（回显使用）",
          value: '用户数组，需包含用户id和username，例如 [{"id":"1","username":"name"}]',
        },
      ],
      data11: [
        {
      ...
      data23: [
        {
          name: "value",
          desc: "绑定的值，可使用 v-model 双向绑定。单张时只接受String，多张时只接受 Array",
          type: "String | Array",
          value: "空",
        },
        {
      ...
      data37: [
        {
          name: "on-change",
          type: "返回上传成功文件对象信息",
          value: "Object：{url: 完整下载链接, name: 文件名, size: 文件大小(单位字节)}",
        },
      ],
      data38: [
        {
      ...
      data40: [
        ...
        {
          name: "auth",
          desc: "是否需要认证的请求，设为true后请求头会携带认证使用的accessToken",
          type: "Boolean",
          value: "true",
        },
        {
