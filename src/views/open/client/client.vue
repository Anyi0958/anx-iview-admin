  ...
  <div class="search">
    ...
    <Modal v-model="modalVisible" :title="modalTitle" :mask-closable="false" :width="500">
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate">
        <FormItem v-if="modalType != 0" label="clientId">{{ form.id }}</FormItem>
        <FormItem label="网站名称" prop="name">
...
<script>
import { getSecretKey, getClientDataList, addClient, updateClient, deleteClient } from "@/api/open";
  ...
  data() {
    return {
      ...
      formValidate: {
        ...
        clientSecret: [{ required: true, message: "不能为空", trigger: "change" }],
        homeUri: [
          {
            required: true,
            type: "url",
