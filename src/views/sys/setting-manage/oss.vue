      ...
      <div v-if="oss.serviceName != 'LOCAL_OSS'">
        ...
        <FormItem label="endpoint域名" prop="endpoint" :error="errorMsg">
          <Input
            ...
            :disabled="changeLoading"
          >
            <Select slot="prepend" v-model="oss.http" style="width: 80px" prop="http">
  ...
  data() {
    return {
      ...
      ossValidate: {
        ...
        zone: [{ required: true, type: "number", message: "不能为空", trigger: "change" }],
        bucketRegion: [{ required: true, message: "不能为空", trigger: "change" }],
      },
    };
  },
  mounted() {
