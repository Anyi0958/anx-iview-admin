  ...
  <div class="search">
    <Card>
      <Row class="operation">
        ...
        <Button icon="md-add" @click="addRoot">添加顶部菜单</Button>
        <Button icon="md-trash" @click="delAll">批量删除</Button>
        <Button v-show="showType == 'list'" icon="md-refresh" @click="getParentList">刷新</Button>
        <Dropdown v-show="showType == 'tree'" @on-click="handleDropdown">
          <Button>
            更多操作
      ...
      <Row v-show="showType == 'tree'" type="flex" justify="start" :gutter="16">
        ...
        <Col :sm="16" :md="16" :lg="16" :xl="9">
          <Form ref="form" :model="form" :label-width="110" :rules="formValidate">
            ...
            <FormItem v-if="form.type == 1" label="按钮权限类型" prop="buttonType">
              <Select v-model="form.buttonType" placeholder="请选择或输入搜索" filterable clearable>
                <Option v-for="(item, i) in dictPermissions" :key="i" :value="item.value">
                  {{ item.title }}
                </Option>
              </Select>
            </FormItem>
            ...
            <FormItem v-if="form.type == -1" label="系统站内菜单" prop="isMenu">
              <i-switch v-model="form.isMenu" size="large">
                <span slot="open">是</span>
                <span slot="close">否</span>
              </i-switch>
            </FormItem>
            <FormItem v-if="form.type == -1 && !form.isMenu" label="其他链接" prop="url">
              <Input v-model="form.url" placeholder="Http链接或路由名" />
            </FormItem>
            <FormItem v-if="form.type == -1 && !form.isMenu" label="打开方式" prop="description">
            ...
            <FormItem v-if="form.type == 0 && form.level == 2" label="第三方链接" prop="url">
            ...
            <FormItem v-if="form.level == 1" label="始终显示" prop="showAlways" class="block-tool">
    ...
    >
      <Form ref="formAdd" :model="formAdd" :label-width="110" :rules="formValidate">
        ...
        <FormItem v-if="formAdd.type == 1" label="按钮权限类型" prop="buttonType">
        ...
        <FormItem v-if="formAdd.type == -1" label="系统站内菜单" prop="isMenu">
          <i-switch v-model="formAdd.isMenu" size="large">
            <span slot="open">是</span>
            <span slot="close">否</span>
          </i-switch>
        </FormItem>
        <FormItem v-if="formAdd.type == -1 && !formAdd.isMenu" label="其他链接" prop="url">
          <Input v-model="formAdd.url" placeholder="Http链接或路由名" />
        </FormItem>
        <FormItem v-if="formAdd.type == -1 && !formAdd.isMenu" label="打开方式" prop="description">
        ...
        <FormItem v-if="formAdd.type == 0 && formAdd.level == 2" label="第三方链接" prop="url">
        ...
        <FormItem v-if="formAdd.level == 1" label="始终显示" prop="showAlways" class="block-tool">
  ...
  data() {
    return {
      ...
      formValidate: {
        ...
        name: [{ required: true, message: "路由英文名不能为空", trigger: "change" }],
        icon: [{ required: true, message: "图标不能为空", trigger: "change" }],
        path: [{ required: true, message: "路径不能为空", trigger: "change" }],
        component: [{ required: true, message: "前端组件不能为空", trigger: "change" }],
        sortOrder: [
          {
            required: true,
            type: "number",
