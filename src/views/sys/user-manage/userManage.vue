...
<template>
  <div class="search">
    <Card>
        ...
        <Form ref="searchForm" :model="searchForm" inline :label-width="70">
          ...
          <span v-if="drop">
            ...
            <FormItem label="性别" prop="sex">
              <dict v-model="searchForm.sex" dict="sex" style="width: 200px" />
            </FormItem>
      ...
      <Row class="operation">
        <Button type="primary" icon="md-add" @click="add">添加用户</Button>
    ...
    <Drawer v-model="importModalVisible" title="导入数据" closable width="1000">
      <div style="display: flex; align-items: center; justify-content: space-between">
        ...
        <Button icon="md-trash" @click="clearImportData">清空数据</Button>
      </div>
      ...
      <div class="drawer-footer">
        <Button type="info" @click="downloadTemple">下载导入模板</Button>
    ...
    <addEdit v-model="showUser" :data="form" :type="showType" @on-submit="getDataList" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    resetPass() {
      this.$Modal.confirm({
        title: "确认重置",
        content: "您确认要重置所选的 " + this.selectList.length + " 条用户数据密码为【123456】?",
        loading: true,
        onOk: () => {
          let ids = "";
    ...
    beforeUploadImport(file) {
      ...
      if (fileExt == "xlsx" || fileExt == "xls") {
        ...
        this.$Notice.warning({
          title: "文件类型错误",
          desc:
            "所选文件‘ " + file.name + " ’不是EXCEL文件，请选择后缀为.xlsx或者.xls的EXCEL文件。",
        });
      }
      return false;
    },
