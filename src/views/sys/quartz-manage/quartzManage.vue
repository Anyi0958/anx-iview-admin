...
<template>
  <div class="search">
    <Card>
      <Row class="operation">
        <Button type="primary" icon="md-add" @click="addRole">添加新任务</Button>
    ...
    <Modal v-model="modalVisible" draggable :title="modalTitle" :mask-closable="false" :width="500">
      ...
      <div slot="footer">
        ...
        <Button type="primary" :loading="submitLoading" @click="handleSubmit">保存并执行</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      formValidate: {
        jobClassName: [{ required: true, message: "任务类名不能为空", trigger: "change" }],
        cronExpression: [{ required: true, message: "cron表达式不能为空", trigger: "change" }],
      },
      submitLoading: false,
      selectList: [],
      columns: [
