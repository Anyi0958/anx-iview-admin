...
<template>
  <div class="search">
    ...
    <Card v-show="!showHistory">
      ...
      <Row class="operation">
        <Button icon="md-checkmark-circle-outline" @click="passAllTask">批量通过</Button>
        <Button icon="md-close" @click="backAllTask">批量驳回</Button>
    ...
    <!-- 审批操作 -->
    <Modal v-model="modalTaskVisible" :title="modalTaskTitle" :mask-closable="false" :width="500">
      <Form ref="form" :model="form" :label-width="85" :rules="formValidate">
        <FormItem label="审批意见" prop="reason">
          <Input v-model="form.comment" type="textarea" :rows="4" />
        </FormItem>
        <FormItem v-show="showAssign" label="下一审批人" prop="assignees" :error="error">
          <Select
        ...
        <div v-show="form.type == 1">
          <FormItem label="驳回至">
            ...
            >
              <Option v-for="(item, i) in backList" :key="i" :value="item.key">
                {{ item.name }}
              </Option>
            </Select>
          </FormItem>
        ...
        <FormItem v-show="form.type == 2" label="选择转办人" prop="userId" :error="error">
          <Select
            v-model="form.userId"
            placeholder="请输入用户名搜索选择用户"
            filterable
