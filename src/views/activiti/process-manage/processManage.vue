...
<template>
  <div class="search">
    ...
    <Card v-show="!showProcessNodeEdit">
      ...
      <Row class="operation">
        <Button type="primary" icon="md-cloud-upload" @click="deploy">部署流程文件</Button>
    ...
    <!-- 图片预览 -->
    <Modal v-model="viewImage" :title="viewTitle" draggable width="800">
      <img
        :src="diagramUrl"
        alt="无效的图片链接"
      ...
      />
      <div slot="footer">
        <Button style="margin-right: 5px" @click="viewImage = false">关闭</Button>
...
export default {
  ...
  methods: {
    ...
    showResource(type, v) {
      if (type == 0) {
        window.open(
          `${exportResource}?id=${v.id}&type=${type}&accessToken=${this.getStore("accessToken")}`
        );
      } else if (type == 1) {
        this.viewTitle = "流程图片预览(" + v.diagramName + ")";
        this.diagramUrl = `${exportResource}?id=${v.id}&type=${type}&accessToken=${this.getStore(
          "accessToken"
        )}`;
        this.viewImage = true;
      }
