...
<template>
  <div class="search">
    <historicDetail v-if="showHistory" :proc-inst-id="procInstId" @close="showHistory = false" />
    <Card v-show="!showHistory">
      ...
      <Row class="operation">
        <Button type="primary" icon="md-add" @click="add">新增申请</Button>
    ...
    <!-- Drawer抽屉式选择流程 -->
    <Drawer v-model="processModalVisible" title="选择流程" closable width="820" draggable>
        ...
        <div>
          <Form ref="searchProcessForm" :model="searchProcessForm" inline :label-width="70">
            ...
            <FormItem style="margin-left: -70px" class="br">
              <Button type="primary" icon="ios-search" @click="searchProcess">搜索</Button>
    ...
    <Modal v-model="modalVisible" title="提交申请" :mask-closable="false" :width="500">
      <Form ref="form" :model="form" :label-width="85" :rules="formValidate">
        <FormItem v-show="showAssign" label="选择审批人" prop="assignees" :error="error">
          <Select
        ...
        <FormItem v-show="isCustom" label="自定义搜索选择审批人" prop="assignees" :error="error">
          <Select
            v-model="form.assignees"
            ...
            :loading="userLoading"
          >
            <Option v-for="(item, i) in userList" :key="i" :value="item.id" :label="item.nickname">
              <span style="margin-right: 10px">{{ item.nickname }}</span>
              <span style="color: #ccc">{{ item.username }}</span>
            </Option>
          </Select>
        </FormItem>
        <FormItem v-show="isGateway" label="下一审批人">
          <span>分支网关处暂不支持自定义选择下一审批人，将发送给下一节点所有人</span>
        </FormItem>
        <FormItem label="优先级" prop="priority">
          <dict v-model="form.priority" dict="priority" />
        </FormItem>
        <FormItem label="消息通知">
    ...
    <Modal v-model="modalCancelVisible" title="确认撤回" :mask-closable="false" :width="500">
      <Form ref="delForm" v-model="cancelForm" :label-width="70">
        <FormItem label="撤回原因" prop="reason">
          <Input v-model="cancelForm.reason" type="textarea" :rows="4" />
        </FormItem>
      </Form>
      <div slot="footer">
