...
<template>
  <div class="search">
    <Card>
      ...
      <Row class="operation">
        <Button type="primary" icon="md-add" @click="add">添加空白模型</Button>
    ...
    >
      ...
      <div style="position: relative; height: 100%">
        <iframe
          ...
          scrolling="auto"
        ></iframe>
        <Spin v-if="modelerLoading" fix size="large"></Spin>
      </div>
    </Modal>
  </div>
</template>
...
<script>
import { getModelDataList, addModel, deployModel, exportModel, deleteModel } from "@/api/activiti";
...
export default {
  ...
  methods: {
    ...
    edit(v) {
      ...
      this.modelerUrl = `${this.domain}/modeler.html?modelId=${v.id}&accessToken=${this.getStore(
        "accessToken"
      )}&time=${new Date()}`;
    ...
    showJump() {
      this.$Modal.confirm({
        title: "部署发布成功",
        content: "建议立即去分配审批节点人员并编辑流程信息，是否立即跳转查看该流程?",
        onOk: () => {
          this.$router.push({
            name: "process-manage",
          });
    ...
    export(v) {
      window.open(exportModel + v.id + "?accessToken=" + this.getStore("accessToken"));
    },
    remove(v) {
      this.$Modal.confirm({
        title: "确认删除",
