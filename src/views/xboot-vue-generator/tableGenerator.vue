  ...
  <div class="search">
    <Card>
      <Row class="operation">
        ...
        <span>表单显示：</span>
        <Select v-model="rowNum" transfer style="width: 150px; margin-right: 10px">
          <Option value="1">一行一列</Option>
          <Option value="2">一行两列</Option>
          <Option value="3">一行三列</Option>
          <Option value="4">一行四列</Option>
        </Select>
        <Button type="primary" icon="md-add" @click="add">添加字段</Button>
        <Button type="warning" icon="ios-construct" @click="entityModal = true">读取字段</Button>
        <Button :loading="loading" type="success" icon="md-send" @click="generate">生成代码</Button>
        <Button icon="md-trash" @click="clear">清空全部</Button>
      </Row>
      <Row>
        <Alert show-icon>
          将生成的代码复制粘贴至XBoot前端新建的空白组件中，再做少许修改并自行调通接口即可
          <span class="preview" @click="preview">增删改表格预览</span>
        </Alert>
      </Row>
      <Row>
        <Table ref="table" border :columns="columns" :data="data">
          ...
          <template slot="tableShow" slot-scope="{ row }">
          ...
          <template slot="searchable" slot-scope="{ row }">
            <i-switch v-model="row.searchable" @on-change="changeSearchable(row, $event)">
              <span slot="open">开</span>
              <span slot="close">关</span>
            </i-switch>
          </template>
          <template slot="searchType" slot-scope="{ row }">
            <Select
          ...
          <template slot="sortable" slot-scope="{ row }">
    ...
    <Modal v-model="modalVisible" :title="modalTitle" :mask-closable="false" :width="750">
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate" inline>
        ...
        <FormItem v-show="form.editable" label="字段表单类型" prop="type">
        ...
        <FormItem v-show="form.type == 'dict'" label="字典类型" prop="dictType">
          <customList
        ...
        <FormItem v-show="form.type == 'customList'" label="自定义URL" prop="customUrl">
          <Input
        ...
        <FormItem v-show="form.type == 'area'" label="显示级别" prop="level">
          <Select v-model="form.level" transfer style="width: 245px">
            <Option value="0">仅显示省</Option>
            <Option value="1">显示省和县</Option>
            <Option value="2">显示省市县</Option>
          </Select>
        </FormItem>
        <FormItem v-show="form.editable" label="是否必填" prop="validate" style="width: 345px">
        ...
        <FormItem v-show="form.searchable" label="搜索框类型" prop="searchType">
        ...
        <FormItem v-show="form.searchType == 'dict'" label="字典类型" prop="searchDictType">
          <customList
        ...
        <FormItem v-show="form.searchType == 'customList'" label="自定义URL" prop="searchCustomUrl">
          <Input
        ...
        <FormItem v-show="form.searchType == 'area'" label="显示级别" prop="searchLevel">
        ...
        <FormItem v-show="form.sortable" label="默认排序" prop="defaultSort" style="width: 345px">
    ...
    >
      ...
      <Tabs v-model="tabName" type="card" @on-click="changeCode">
        ...
        <TabPane
          v-if="
            resultType == 'drawerApi' || resultType == 'componentApi' || resultType == 'singleApi'
          "
          name="api.js"
          label="api.js"
        ></TabPane>
    ...
    <Modal v-model="entityModal" title="自动读取字段" :width="700" :fullscreen="fullEntity">
      <Alert show-icon>
      ...
      <div slot="footer">
        <Button icon="md-expand" @click="fullEntity = !fullEntity">全屏开/关</Button>
        <Button type="primary" icon="md-checkmark-circle-outline" @click="submitEntityData">
          确认更新字段值
        </Button>
      </div>
    </Modal>
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.daterangeSearch && data.searchType == "daterange") {
            this.$Message.warning("已设定了日期范围搜索的字段，日期范围搜索暂仅支持设定1个");
            return;
          }
          if (this.modalType == 0) {
            this.data.push(data);
