  ...
  <div class="search">
    <Card>
      ...
      <Row>
        <Alert show-icon>
          ...
          <span class="preview" @click="preview">树形结构预览</span>
    ...
    <Modal v-model="modalVisible" :title="modalTitle" :mask-closable="false" :width="500">
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate">
        ...
        <FormItem v-show="form.type == 'customList'" label="自定义URL" prop="customUrl">
          <Input v-model="form.customUrl" placeholder="输入自定义选择组件url属性" />
        </FormItem>
        <FormItem v-show="form.type == 'area'" label="显示级别" prop="level">
          <Select v-model="form.level" transfer>
            <Option value="0">仅显示省</Option>
            <Option value="1">显示省和县</Option>
            <Option value="2">显示省市县</Option>
          </Select>
        </FormItem>
        <FormItem v-show="form.editable" label="是否必填" prop="validate">
    ...
    >
      <RadioGroup v-model="resultType" style="margin-bottom: 15px" @on-change="changeRadio">
        <Radio label="drawerApi" border>树结构+抽屉(含API)</Radio>
      ...
      if (flag) {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            generateTree(this.vueName, this.rowNum, this.enableTable, this.data).then(res => {
              this.loading = false;
              if (res.success) {
